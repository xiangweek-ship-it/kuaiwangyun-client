...
class ApiAdapter implements HttpClientAdapter {
  ...
  Future<ResponseBody> fetch(RequestOptions options, Stream<Uint8List>? requestStream, Future<void>? cancelFuture) async {
    return ResponseBody.fromString(jsonEncode(handler(options)), 200,
...
Map<String, dynamic> payload({bool active = true, String expiry = '2030-01-01T00:00:00+08:00'}) => {
  ...
  'sources': [{'id': 1, 'name': 'Route A', 'url': 'https://kuaiwangyun.com/vpn/user/subscribe.php?key=${'a' * 64}&source=1&client=clash'}],
...
void main() {
  test('membership eligibility uses server time and account status', () {
    ...
    expect(ClientAccount.fromJson(payload(expiry: '2020-01-01T00:00:00Z')).canConnect, isFalse);
  });
  test('only HTTPS first-party subscription endpoints are accepted', () {
    for (final url in ['http://kuaiwangyun.com/vpn/user/subscribe.php', 'https://evil.example/vpn/user/subscribe.php', 'https://kuaiwangyun.com/other', 'https://name:secret@kuaiwangyun.com/vpn/user/subscribe.php', 'https://kuaiwangyun.com:8443/vpn/user/subscribe.php']) {
      expect(() => ClientSource.fromJson({'id': 1, 'name': 'A', 'url': url}), throwsA(isA<ClientApiException>()));
    }
  });
  test('login stores only bearer token and logout deletes it', () async {
    ...
    dio.httpClientAdapter = ApiAdapter((options) {
      ...
      return options.queryParameters['action'] == 'logout' ? {'ok': true} : {...payload(), 'access_token': token};
    });
    final api = ClientApi(dio: dio, storage: storage);
    await api.login(' qa ', 'not-persisted');
    await api.account();
    await api.logout();
    verify(() => storage.write(key: 'kuaiwangyun.session.v1', value: token)).called(1);
